...
if strcmp(method, 'qm') 
    ...
    for i = 1:size(x_mdl_h, 2)
        for j = 1:size(x_mdl_h, 3)
            y_mdl_p(:, i, j) = interp1(squeeze(x_mdl_h(:, i, j)), ...
                                       squeeze(y_mdl_h(:, i, j)), ...
                                       squeeze(mdl_p(:, i, j)));
            obs_p(:, i, j) = quantile(obs_h(:, i, j), y_mdl_p(:, i, j));
        end
    end
...
function [FX, X] = cdf_transform(x, method, varargin)
    ...
    if strcmp(method, 'ecdf')
        ...
        FX = (dst - 0.44)/(size(x, 1) + 0.12);
        X  = x;
        
    elseif strcmp(method, 'ecdf2')
        [FX, X] = ecdf(x);
        FX      = FX(2:end);
        X       = X(2:end);
