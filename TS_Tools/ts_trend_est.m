function [ts_out, a, b, sig_p] = ts_trend_est(ts_in, alpha, wghts, est_method, test_method)
...
nts     = size(ts_in.TimeStamp, 1);
nvars   = size(ts_in.Data, 2);
est_vec = (1:nts)';
...
elseif strcmp(est_method, 'TS')
    ...
    mask                     = zeros(nts, nvars);
    mask(~isnan(ts_in.Data)) = 1;
    nts_nonan                = sum(mask);
    
    % Accumulate slopes
    for i = 1:nts
        den = repmat(ts_in.Data(i, :), nts, 1) - ts_in.Data;
        num = (est_vec(i) - est_vec)*ones(1, nvars);
    ...
    x_hat(2, :) = nanmedian(ts_in.Data - est_vec*x_hat(1, :));
