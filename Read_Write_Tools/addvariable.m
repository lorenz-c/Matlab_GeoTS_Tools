...
elseif ismember(vartype, {'2d_grids_projected', '3d_grids_projected'})
    
     if ~ismember(vars, 'x')
        % Add x to inpt
        otpt.Variables.x.standard_name = 'projection_x_coordinate';
        otpt.Variables.x.units         = 'm';
        otpt.Variabkes.x.axis          = 'X';
...
elseif strcmp(vartype, '2d_stations')
    ...
    if ~ismember(vars, 'stations')
        ...
        otpt.Variables.stations.coordinates   = char.empty(0);
...
elseif ismember(vartype, {'2d_grids', '2d_stations', '2d_regions', '2d_grids_projected'}) 
    gridsize = [0 0];
elseif ismember(vartype, {'3d_grids', '3d_grids_projected'})
    gridsize = [0 0 0];
...
if ismember(vartype, {'1d_stations', '2d_stations', '1d_regions', ...
               '2d_regions', '3d_grids', '3d_grids_projected', '4d_grids'})
...
elseif strcmp(vartype, '2d_grids_projected')
    datadims = {'y', 'x'};
    otpt.DataInfo.cdm_data_type = 'Grid';
elseif strcmp(vartype, '3d_grids_projected')
    datadims = {'time', 'y', 'x'};
    otpt.DataInfo.cdm_data_type = 'Grid';
end
...
for i = 1:length(varnames)   
    ...
    if ismember(vartype, {'2d_grids_projected', '3d_grids_projected'})
